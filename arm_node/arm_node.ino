...
void joyControls( const sensor_msgs::Joy &msg){
  ...
  if(msg.axes[0]<=-0.5){          //Movimiento hacia la izquierda
    digitalWrite(in1_Base, LOW);
    digitalWrite(in2_Base, HIGH);
  }
  else if(msg.axes[0]>=0.5){      //Movimiento hacia la derecha
  ...
  if(msg.axes[2]<=-0.5){          //Movimiento hacia abajo
    digitalWrite(in1_Codo, LOW);
    digitalWrite(in2_Codo, HIGH);       
  }
  else if(msg.axes[2]>=0.5){       //Movimiento hacia arriba
  ...
  //Motor Pinza
  if(msg.buttons[0]==1 && msg.buttons[1]==0){      //Abrir
    it6 = it6 - 4;
    delay(50);
    if(it6<30)
      it6 = 30;       
  }
  else if(msg.buttons[1]==1 && msg.buttons[0]==0){ //Cerrar
    it6 = it6 + 4;
    delay(50);
    if(it6>100)
      it6 = 100;  
